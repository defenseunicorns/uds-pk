...
func TestSetupTables(t *testing.T) {
	...
	if err := newTable.Append(dummyRow); err != nil {
		t.Errorf("Failed to append to newTable: %v", err)
	}
	if err := fixedTable.Append(dummyRow); err != nil {
		t.Errorf("Failed to append to fixedTable: %v", err)
	}
	if err := existingTable.Append(dummyRow); err != nil {
		t.Errorf("Failed to append to existingTable: %v", err)
	}

	if err := newTable.Render(); err != nil {
		t.Errorf("Failed to render newTable: %v", err)
	}
	if err := fixedTable.Render(); err != nil {
		t.Errorf("Failed to render fixedTable: %v", err)
	}
	if err := existingTable.Render(); err != nil {
		t.Errorf("Failed to render existingTable: %v", err)
	}
